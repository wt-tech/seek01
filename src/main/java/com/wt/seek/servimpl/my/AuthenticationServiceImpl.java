	...
	public boolean updateAuthentication(Authentication authentication, MultipartFile negativIdentityUrl,
		...
		if (authentication.getId() > 0) { // 保存成功
			if (null != file) {
				...
				String absolutePath = ImageUtils.generateAbsoluteImgPath(staticsPath, Constants.AUTHENTICATION, authentication.getId(),
						suffix);
				// 上传图片
				flag= ImageUtils.saveImage(file, absolutePath);
				String url = ImageUtils.genrateVirtualImgPath(Constants.AUTHENTICATION, authentication.getId(), suffix);
				if (flag) {// 图片存储成功
					if (null != negativIdentityUrl)
